...
def ask_expert(question, name, company, bio, last_conversation, chat_log=None):
    ...
    if chat_log is None:
        initial_prompt, recurrent_prompt = generate_prompts(name, company, bio, last_conversation)
...
def chat():
    ...
    if request.method == 'POST' and 'user_msg' in request.form:
        ...
        response, chat_log = ask_expert(question, name, company, bio, last_conversation, session.get('chat_log'))
